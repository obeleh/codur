...
def mock_tools_with_side_effects():
    ...
    with patch(
        "codur.utils.message_pipeline.get_tools_with_side_effects",
...
class TestNonMutationToolCallsBeforeSystemMessage:
    """Non-mutation tool calls before first SystemMessage."""

    def test_non_mutation_tools_before_system_message_are_preserved(self):
        """Non-mutation tool calls before SystemMessage are preserved."""
        messages = [
        ...
        # Without mutation, all tool calls are injected after system message
        assert len(result) == 4
    ...
    def test_multiple_non_mutation_tools_before_system_message(self):
        ...
        # All 4 tool calls + system message
        assert len(result) == 5
        assert isinstance(result[0], SystemMessage)
...
class TestMutationToolCallInMiddle:
    ...
    def test_mutation_tool_preserves_subsequent_tool_calls(self):
        ...
        # SystemMessage + tool call after mutation + HumanMessage
        assert len(result) == 3
        assert isinstance(result[0], SystemMessage)
    ...
    def test_mutation_in_middle_drops_before_preserves_after(self):
        ...
        # SystemMessage + 2 tool calls after mutation
        assert len(result) == 3
        assert isinstance(result[0], SystemMessage)
...
class TestMultipleSystemMessages:
    """Multiple SystemMessages with tool calls in between."""

    def test_only_last_system_message_is_kept(self):
        """Only the last SystemMessage is preserved, with tool calls injected."""
        messages = [
            SystemMessage(content="first context"),
            HumanMessage(content="task 1"),
            make_tool_message("read_file"),
            SystemMessage(content="second context"),
            HumanMessage(content="task 2"),
            make_tool_message("list_files"),
            SystemMessage(content="third context"),
            HumanMessage(content="task 3"),
        ]

        result = message_shortening_pipeline(messages)

        # Last SystemMessage + all tool calls before it + HumanMessage after it
        assert len(result) == 4
        assert isinstance(result[0], SystemMessage)
    ...
    def test_tool_calls_between_system_messages_without_mutation(self):
        """Tool calls between SystemMessages without mutation are preserved."""
        messages = [
            SystemMessage(content="ctx1"),
            make_tool_message("read_file", "read1"),
            SystemMessage(content="ctx2"),
            make_tool_message("list_files", "list1"),
            SystemMessage(content="ctx3"),
        ]

        result = message_shortening_pipeline(messages)

        # All tool calls before last system message are injected
        assert len(result) == 3
...
class TestMultipleSystemMessagesWithMutation:
    """Multiple SystemMessages with mutating tool calls in between."""

    def test_mutation_before_last_system_message_preserves_subsequent_tools(self):
        """Mutation before last SystemMessage preserves tools after it."""
        messages = [
            SystemMessage(content="ctx1"),
            make_tool_message("write_file"),  # mutation
        ...
        # Last system message + tool after mutation + HumanMessage
        assert len(result) == 3
    ...
    def test_multiple_mutations_only_last_matters(self):
        ...
        # Last system message + only tool after last mutation
        assert len(result) == 2
        assert result[0].content == "ctx2"
        assert result[1].content == "after_second"

    def test_mutation_after_last_system_message(self):
        """Mutation after last SystemMessage - all prior tools preserved.

        When mutation is after the last system message, all tool calls from 0 to
        system message are preserved (since there's no mutation before system message).
        """
        messages = [
            SystemMessage(content="ctx1"),
            make_tool_message("read_file", "before"),
            SystemMessage(content="ctx2"),
            make_tool_message("write_file"),  # mutation after last system message
        ...
        # All tool calls before system message are preserved + messages after system message
        assert len(result) == 5
        assert result[0].content == "ctx2"
        assert result[1].content == "before"  # tool call before system message
        assert result[2].name == "write_file"  # from messages after system message
...
class TestCornerCases:
    ...
    def test_no_system_message_raises_error(self):
        """Missing SystemMessage raises ValueError."""
        messages = [
            HumanMessage(content="task"),
            make_tool_message("read_file"),
        ]

        with pytest.raises(ValueError, match="No SystemMessage found"):
    ...
    def test_only_system_message(self):
        """Single SystemMessage returns just that message."""
        messages = [SystemMessage(content="only")]
    ...
    def test_system_message_at_end(self):
        """SystemMessage at end of message list with tool calls preserved."""
        messages = [
            HumanMessage(content="task"),
            AIMessage(content="response"),
            make_tool_message("read_file"),
            SystemMessage(content="final context"),
        ]

        result = message_shortening_pipeline(messages)

        # Tool call before system message is preserved
        assert len(result) == 2
        assert result[0].content == "final context"
        assert result[1].name == "read_file"

    def test_ai_messages_after_system_message_preserved(self):
        """Non-tool messages after SystemMessage are preserved."""
        messages = [
            make_tool_message("read_file"),
            SystemMessage(content="context"),
        ...
        # Tool call before system message is also preserved
        assert len(result) == 5
        assert isinstance(result[0], SystemMessage)
        assert result[1].name == "read_file"  # tool call injected
    ...
    def test_mutation_at_very_end(self):
        """Mutation tool as last message (after system message)."""
        messages = [
            SystemMessage(content="context"),
        ...
        # All messages after system message are preserved
        assert len(result) == 3
    ...
    def test_no_mutations_all_tool_calls_preserved(self):
        """Without mutations, all tool calls before system message are preserved."""
        messages = [
    ...
    def test_consecutive_mutations(self):
        ...
        # Only tool after last mutation + system message
        assert len(result) == 2
        assert result[0].content == "context"
    ...
    def test_mixed_message_types_complex(self):
        ...
        # Last system message + tool after mutation (verify) + messages after system message
        assert len(result) == 4
        assert result[0].content == "ctx2"
        assert result[1].content == "verify"  # Tool after mutation
        assert result[2].content == "listing"  # Tool after last system message
        assert result[3].content == "final"

    def test_tool_calls_only_after_mutation_before_system_message(self):
        """Tool calls between mutation and system message are preserved."""
        messages = [
        ...
        # System message + 2 tool calls after mutation
        assert len(result) == 3
        assert result[0].content == "context"
